...
from agent.loops.post_morning_review import build_l2_calibration_coverage, run_post_morning_review, select_run_session_events
...
def test_parse_monitor_logs_builds_morning_acceptance_summary():
    ...
    lines = [
        "MONITOR_SESSION pool_generated output=data/stock_pools/current/main_seal_follow_pool.csv source=combined total=52 amount=1000.0",
        "MONITOR_SESSION monitor_start csv=data/stock_pools/current/main_seal_follow_pool.csv stop_time=12:00 dry_run=True summary_mode=True",
        "MONITOR_SESSION session_start mode=market-only-monitor dry_run=True csv=data/stock_pools/current/main_seal_follow_pool.csv stop_at=12:00 account_connected=false",
...
def test_parse_monitor_logs_counts_reused_pool_from_strategy_selection():
    lines = [
        "MONITOR_SESSION pool_reused csv=data/stock_pools/manual/main_seal_follow_manual_pool.csv source=manual total=unchanged amount=50000.0",
        "MONITOR_SESSION monitor_start csv=data/stock_pools/manual/main_seal_follow_manual_pool.csv stop_time=11:00 dry_run=True summary_mode=True",
        "STRATEGY_SELECTION strategy=MainSealFollowStrategy mode=subprocess total=1 csv_path=data/stock_pools/manual/main_seal_follow_manual_pool.csv stocks=600162",
        "Runtime heartbeat mode=market-only-monitor dry_run=True connected=True strategies=1 tick_subscriptions=0 l2_stocks=1 l2_kinds=1 latest_data_time=2026-06-03T09:15:00 data_delay_ms=100 process_ms=0.0",
        "MONITOR_SESSION session_stop reason=scheduled_stop stop_time=11:00 strategy_count=1 tick_subscriptions=0 l2_stocks=1",
    ]

    summary = summarize_events(_event(line) for line in lines)

    assert summary["pool_total"] == 1
    assert summary["selected_stocks"] == ["600162"]
    assert summary["checks"]["pool_generated"] is True
    assert summary["event_counts"]["strategy_selection"] == 1


def test_parse_monitor_logs_falls_back_to_strategy_init_stocks():
    lines = [
        "MONITOR_SESSION pool_reused csv=data/stock_pools/manual/main_seal_follow_manual_pool.csv source=manual total=unchanged amount=50000.0",
        "Strategy[abc] MainSealFollow 初始化 stock=600162",
        "Strategy[def] MainSealFollow 初始化 stock=600604",
    ]

    summary = summarize_events(_event(line) for line in lines)

    assert summary["selected_stocks"] == ["600162", "600604"]


def test_l2_calibration_coverage_reports_missing_files(tmp_path: Path):
    calibration_dir = tmp_path / "l2_calibration"
    calibration_dir.mkdir()
    (calibration_dir / "main_seal_follow_600162_香江控股.jsonl").write_text("{}\n", encoding="utf-8")

    coverage = build_l2_calibration_coverage(
        summary={"selected_stocks": ["600162", "600604"]},
    ...
    assert coverage["missing_stocks"] == ["600604"]
...
def test_generate_tasks_for_active_strategies_without_signals():
    ...
    assert dump_tasks_yaml(tasks).startswith("# 由 agent.loops.generate_improvement_tasks 自动生成")
