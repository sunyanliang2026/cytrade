...
The original strategy under ``docs/original_juejin_main.py`` is a sell-only
strategy driven by tick-level bid/ask prices and a small CSV file. This module
keeps the same state-machine shape while routing all orders through the
project's ``TradeExecutor`` so dry-run/live safety gates remain centralized.
"""
...
class JuejinSellStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        self._csv_position = self._build_csv_position()

    # ------------------------------------------------------------------ Selection

    def select_stocks(self) -> List[StrategyConfig]:
        """Read Juejin's ``sell_10.csv`` and create one strategy per row."""
        csv_path = self._resolve_csv_path(self.config.params.get("csv_path"))
    ...
    def on_order_update(self, order: Order) -> None:
        """Keep sell-side verification running when the account has no holding.

        The expected test-stage failure mode is that QMT rejects the submitted
        sell order because the account does not actually hold the CSV symbol.
        That rejection proves the trigger reached the execution layer, so this
        strategy records the order update but does not pause only for the
        account-position rejection class.
        """
        if (
            getattr(order, "strategy_id", "") == self.strategy_id
            and getattr(order, "direction", None) == OrderDirection.SELL
            and getattr(order, "status", None) == OrderStatus.JUNK
            and self._is_account_position_rejection(getattr(order, "status_msg", ""))
        ):
            original_status_msg = str(getattr(order, "status_msg", "") or "")
            order.status_msg = "juejin_sell_account_position_rejection_ignored"
            try:
                super().on_order_update(order)
            finally:
                order.status_msg = original_status_msg
            logger.info(
                "JuejinSellStrategy[%s] 卖单因账户持仓不足被拒，验证阶段不暂停 stock=%s msg=%s",
                self.strategy_id[:8],
                self.stock_code,
                original_status_msg,
            )
    ...
    def on_tick(self, tick: TickData) -> Optional[dict]:
        ...
        if (
            ...
            if self._order_submission_accepted(first_order) or remaining_qty > 0:
                self._flag = 1
        ...
        if self._pending_limit_open_second_sell and not self._has_active_sell_orders():
            self._pending_limit_open_second_sell = False
            self._submit_limit_open_second_sell(bid_p, position)
        ...
        if (
            ...
            if self._order_submission_accepted(first_order):
                self._flag = 0
        ...
        if (
            ...
            if self._has_active_sell_orders():
                self._pending_limit_open_second_sell = True
                self.request_state_persist(reason=f"juejin_sell_wait_cancel:{self.strategy_id}", min_interval_sec=0.0)
            else:
                self._submit_limit_open_second_sell(bid_p, position)
    ...
    def _get_custom_state(self) -> dict:
        return {
            ...
            "pending_limit_open_second_sell": self._pending_limit_open_second_sell,
    ...
    def _restore_custom_state(self, state: dict) -> None:
        ...
        self._pending_limit_open_second_sell = bool(
            payload.get("pending_limit_open_second_sell", self._pending_limit_open_second_sell)
        )
    ...
    def _submit_sell(self, quantity: Optional[int], price: float, remark: str, action_key: str = "") -> Optional[Order]:
        ...
        requested_quantity = available if quantity is None else max(0, int(quantity or 0))
        sell_quantity = min(requested_quantity, available)
        ...
        if action_key and self._order_submission_accepted(order):
    ...
    def _submit_limit_open_second_sell(self, bid_price: float, position: Optional[PositionInfo]) -> Optional[Order]:
        order = self._submit_sell(
            self._sell_quantity,
            round(bid_price * 0.99, 2),
            "涨停开板首卖后跌破 3% 卖第二笔",
    ...
    @staticmethod
    def _order_submission_accepted(order: Optional[Order]) -> bool:
        return bool(order and order.status not in (OrderStatus.JUNK, OrderStatus.UNKNOWN))
    ...
    def _cancel_active_orders(self, remark: str) -> int:
        ...
        for order_uuid, order in list(self._pending_orders.items()):
            ...
            if bool(cancel_order(order_uuid, remark=remark)):
                if not bool(getattr(self._trade_executor, "live_trading_enabled", False)) and order.is_active():
                    order.status = OrderStatus.CANCELED
                if not bool(getattr(self._trade_executor, "live_trading_enabled", False)):
    ...
    def _has_active_sell_orders(self) -> bool:
        return any(
            order.direction == OrderDirection.SELL and order.is_active()
    ...
    def _build_csv_position(self) -> Optional[PositionInfo]:
        quantity = max(0, int(self._sell_quantity or 0))
        if quantity <= 0 or not self.stock_code:
            return None
        return PositionInfo(
            ...
            sellable_base_quantity=quantity,
    ...
    def _default_csv_path() -> Path:
        return Path(__file__).resolve().parent / "data" / "sell_10.csv"

    @classmethod
    def _resolve_csv_path(cls, value: Any = None) -> Path:
        text = str(value or os.getenv("CYTRADE_JUEJIN_SELL_CSV_PATH", "") or "").strip()
        if text:
            return Path(text).expanduser().resolve()
        return cls._default_csv_path()
    ...
    @staticmethod
    def _is_account_position_rejection(value: Any) -> bool:
        text = str(value or "")
        lowered = text.lower()
        keywords = (
            "可用数量不足",
